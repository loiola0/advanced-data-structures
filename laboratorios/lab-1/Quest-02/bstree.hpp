...
public:
  ...
  //atualizar valor(questão 02)
  bool update(Key key, Val newval){
      if(root&&search(key)){
        BSTreeNode *currentNode = root.get();
        while (key != currentNode->key){//enquanto a chave procurada não for achada, o correntNode vai percorrer até achar
        
       
       
        if (key < currentNode->key){//verifica se a chave está a esquerda do nó
          currentNode = currentNode->left.get();
        }
        else if (key > currentNode->key){//verifica se a chave está a direita do nó
          currentNode = currentNode->right.get();
        }
        
      }
        currentNode->val = newval;//troca o valor encontrado da chave(valor que se procurava) e troca pelo novo valor
        return true;
      }else{
          return false;//retorna falso se não achou o valor ou se a árvore é vazia
      }
  }
