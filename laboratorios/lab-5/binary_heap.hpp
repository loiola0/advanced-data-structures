...
public:
  ...
  // heap is empty
  std::optional<std::pair<priority_type,Element>> priority_element(){
    if (empty()){
      return {};
    }
    else{
      return std::make_pair(data_[0].priority,data_[0].element);
    }
  }
  // extracts the element with highest priority. Does nothing in case
  // heap is empty
  std::optional<std::pair<priority_type,Element>> extract(){
    if (empty()){
      return {};
    }
    else{
      ...
      // overwritten
      Element movedElement {std::move(data_[0].element)};
      priority_type movedPriority {std::move(data_[0].priority)};
      ...
      return std::make_pair(movedPriority,movedElement);
    }
  }
...
//questões 03,04,05,06,08,09
template<typename Element, typename priority_type = unsigned long>
class BinaryMedianHeap{
  private:
   //questão 04.
     BinaryMaxHeap<Element,priority_type> binMax;
     BinaryMinHeap<Element,priority_type> binMin;
  
  //questão 06.
    void maintain_balance(){
      //verifica se a diferença entre as heaps é dois.
       if(binMax.size()==binMin.size()+2){
         auto [prio,elem] = *binMax.extract();
         binMin.insert(prio,elem);
       }else if(binMax.size()+2==binMin.size()){
         auto [prio,elem] = *binMin.extract();
         binMax.insert(prio,elem);
       }   
    }

  public:
  //questão 05.
      BinaryMedianHeap():binMax{},binMin{}
      {}

    //questão 07.
    std::optional<std::pair<priority_type,Element>> median_element(){
      //caso as heaps tenham o mesmo tamanho.
        if(binMax.size()==binMin.size()){
             auto opt = binMax.priority_element();
             //caso a heapMax não seja vazia, por padrão retornamos o element da heapMaxima.
              if(opt){
                auto [prio,elem] = *opt;
                 return std::make_pair(prio,elem);
              }else{
                //caso as duas heaps sejam vazias.
                return {};
              }
        }else{
          //caso a heapMax seja maior que heapMin.
          if(binMax.size()>binMin.size()){
              auto [prio,elem] = *binMax.priority_element();
              return std::make_pair(prio,elem);
          }else{
            //caso a heapMin seja maior que heapMax.
            auto [prio,elem] = *binMin.priority_element();
           return std::make_pair(prio,elem);
          }
        }
    }
    ...
    void insert(priority_type priority, const Element& element){
        ...
        auto opt = median_element();

          if(opt){
            auto [prio,elem] = *opt;

          //caso o elemento a ser inserido tenha uma prioridade menor ou igual que a de median_element.
            if(priority<=prio){
                binMax.insert(priority,element);
           }else{
             binMin.insert(priority,element);
           }
      }else{
        binMax.insert(priority,element);
      }
  ...
  //função para testes
   void test(){
     while(!binMax.empty()){
       auto [pr,el] = *binMax.extract();
       std::cout<<pr<<":"<<el<<std::endl;
   ...
   void test1(){
     while(!binMin.empty()){
       auto [pr,el] = *binMin.extract();
       std::cout<<pr<<":"<<el<<std::endl;
       
     }
   }

  //questão 09: vuldo remove.
  std::optional<std::pair<priority_type,Element>> median_extract(){
    ...
    //e portanto é nela que deve ser feita a remoção.
      ...
      }else if(binMax.size()<binMin.size()){

        auto [prio,elem] = *binMin.extract();
        return std::make_pair(prio,elem);

      }else if((binMax.size()==binMin.size())&&binMax.size()!=0){
      //caso as duas heaps tenham a mesma quantidade de elementos,
      //temos convencionado que median_element() se encontra na heap
      //máxima, e portanto é nela que deve ser feita a remoção.
