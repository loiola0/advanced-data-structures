...
Endian endian_order(){
  ...
  //1000 0000//little endian

  //0000 0001//big endian
...
bool search_binary (std::vector<int> vect, int key){
...
std::optional<std::pair<int,int>> worstCase(std::vector<int> S,int k){
    int size = S.size();
...
void vector_in_order(std::vector<int> &v){
...
std::optional<std::pair<int,int>> search_complement(std::vector<int> &v,int k){
    ...
    bool number;
      ...
      for(int i=0;i<size;i++){
         ...
         number = search_binary(v,k-v[i]);

         if(number){
            return std::make_pair(v[i],k-v[i]);
         }
...
//little C- O(n)
  ...
  std::optional<std::pair<int,int>> usingHashTables(std::vector<int> v,int k){
      HashTable<int, int> h {};
      int size = v.size();
...
std::optional<std::pair<int,int>> has(std::vector<int> v,int k){
     HashTable<int, int> h {};
     int size = v.size();
