...
public:
  ...
  //return the degree(in integer) of output.
  int out_degree(size_type u){
      size_type aux = 0;
      int counter_output = 0;

        while(aux<num_verts){
              
            if(has_edge(u,aux)==true){
                counter_output++;
            }
  ...
  //return the degree(in integer) of input.
    int in_degree(size_type v){
      size_type aux = 0;
      int counter_output = 0;

        while(aux<num_verts){
              
            if(has_edge(aux,v)==true){
                counter_output++;
            }
  ...
  //tree quest.
  //return the sum degree output and degree input.
  ...
  //four quest.
  //save outs degree in vector. 
  std::vector<int> calOutDegree(){
      std::vector<int> v;
      size_t i;
        for(i = 0; i<num_verts; i++){  
          v.push_back(out_degree(i));   
      }
      return v;
  }

  //search bigger number in vector.
  std::pair<int,size_type> search_maximum(std::vector<int> v){
    int bigger = v[0];
    size_type vertex = 0;
    ...
    return std::make_pair(bigger,vertex);
  }

  //search smaller number in vector.
  std::pair<int,size_type> search_minimum(std::vector<int> v){
    int smaller = v[0];
    size_type vertex = 0;
    size_type size = v.size();
    ...
    return std::make_pair(smaller,vertex);
  }

  //calcule maximum out degree.
  std::pair<int,size_type> max_out_degree(){
    return search_maximum(calOutDegree());
  }
  //calcule minimum out degree.
  std::pair<int,size_type> min_out_degree(){
    return search_minimum(calOutDegree());
  }

  
  //save in degree in vector.
  std::vector<int> calInDegree(){
      std::vector<int> v;
      size_t i;

        for(i = 0; i<num_verts; i++){
          v.push_back(in_degree(i));
      }
      return v;
  }

  //calcule maximum in degree.
  std::pair<int,size_type> max_in_degree(){
    return search_maximum(calInDegree());
  }

  //calcule minimum in degree.
  std::pair<int,size_type> min_in_degree(){
    return search_minimum(calInDegree());
  }


  //calcule degree
  std::vector<int> calDegree(){
      std::vector<int> v;
      size_t i;
        for(i = 0; i<num_verts; i++){  
  ...
  //calcule maximum degree.
  std::pair<int,size_type> max_degree(){
    return search_maximum(calDegree());
  }
  //calcule minimum degree.
  std::pair<int,size_type> min_degree(){
    return search_minimum(calDegree());
  }

//five quest
  bool valid_path(Digraph_ d, std::vector<int> vertex){
      int size = vertex.size();
        for(int i = 0;i<size-1;i++){
            if(d.has_edge(vertex[i],vertex[i+1]) == false){
              return false;
            }
        }
        return true;
...
template<typename Function>
void breadth_first_search(const Digraph& D, Digraph::size_type start, Function visitor){
  // we are going to use some colors to represent vertex status: white
  // vertices have not been found yet; gray vertices have been found
  // and are to be visited; black vertices have been visited
  enum class Color{white, gray, black};
  ...
  std::vector<Color> color {D.num_verts, Color::white};
  // stack to control the order in which vertices should be visited
  std::queue<Digraph::size_type> dfs {};
  // procedure to be executed when a vertex is found
  auto start_visit {[&dfs, &color](const auto i) {
                      // it is put in the stack
                      dfs.push(i);
  ...
  while (!dfs.empty()){
    // selects a neighbor to explore
    auto neighbor {neighbor_to_visit(dfs.front())};
    // neighbor is a valid vertex, so it must be explored
    if (neighbor < D.num_verts){
      start_visit(neighbor);
    }
...
public:
  ...
  int out_degree(size_type u){
      return Digraph::out_degree(u);
  }

  int in_degree(size_type v){
      return Digraph::in_degree(v);
  }
